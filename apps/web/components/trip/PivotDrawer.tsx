  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
      ...
      <div className="relative w-full max-w-md rounded-t-2xl bg-base p-6 sm:rounded-2xl">
        <h3 className="font-sora text-lg text-ink-100">Suggest a change</h3>
        <p className="mt-1 font-dm-mono text-xs text-ink-400">
          Replacing: {currentActivityName}
        </p>

        <label className="mt-4 block">
          <span className="font-dm-mono text-xs text-ink-300">Why swap this?</span>
          <textarea
            value={reason}
            onChange={(e) => setReason(e.target.value)}
            className="mt-1 block w-full rounded-lg border border-ink-700 bg-surface px-3 py-2 font-dm-mono text-sm text-ink-100 placeholder:text-ink-500 focus:border-accent focus:outline-none"
            rows={3}
        ...
        <div className="mt-4 flex gap-2">
          ...
          <button
            ...
            className="flex-1 rounded-lg bg-accent px-4 py-2 font-dm-mono text-xs text-white transition hover:bg-accent/90 disabled:opacity-50"
          >
